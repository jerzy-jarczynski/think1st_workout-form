...
export const fetchHolidays = async (): Promise<Holiday[]> => {
  ...
  const country = 'PL';
  const year = '2023';
  const types = ['national_holiday', 'observance']; // Dodajemy nowe typy
  const holidays: Holiday[] = [];

  // Pobieramy dane dla każdego typu
  for (const type of types) {
    ...
    try {
      ...
      // Dodajemy pole type do każdego obiektu, aby łatwo odróżnić typy świąt
      const holidaysWithType = data.map((holiday: Holiday) => ({ ...holiday, type }));
      // Łączymy wyniki dla różnych typów w jedną tablicę
